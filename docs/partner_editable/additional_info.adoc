...
== Create IAM user
To launch the Cloudformation stack for this Quick Start, you’ll need to use a user login with programmatic access to following AWS services:
...
. If you do not already have such an IAM user, create an IAM user from AWS console. In the example below, a user “cfnuser” is created with programmatic and console access.
+
:xrefstyle: short
[#iam_1]
.Create IAM user
...
[start=2]
. To follow AWS best practices, allow access to required services. When setting permissions, choose *Attach existing policies directly*. Then choose roles with full access. Repeat this step for all ten services listed previously.
...
+
TIP: As shown in You can filter the role list by entering a few letters of the role you are looking for.
