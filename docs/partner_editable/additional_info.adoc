...
=== Automatic failover
Deploying this Quick Start with the default parameters configures an Auto Scaling group with UiPath Orchestrator instances and three HAA clustered nodes or Amazon EC2 instances.
...
However, the default implementation doesn’t provide automatic failover in every case. For example, loss of Availability Zones 1 and 2 (containing the primary and secondary HAA nodes) prevents automatic failover to Availability Zone 3. This is because the cluster fails as it loses quorum. In this scenario, you can follow manual disaster recovery steps that include rebuilding the cluster that is missing nodes. (At least two nodes must be available.)
...
For UiPath Orchestrator security best practices, see https://docs.uipath.com/orchestrator/docs/security-best-practices[Security Best Practices].
